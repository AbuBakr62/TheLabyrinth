...
namespace Rendering
{
    RTTI_DEFINITIONS(Spikes)

    Spikes::Spikes(Game& game, Camera& camera, const std::string modelFilename)
	...
	Spikes::Spikes(Game& game, Camera& camera, const std::string modelFilename, const std::wstring ModelDes, int ModelValue)
		: DrawableGameComponent(game, camera),
		mEffect(nullptr), mTechnique(nullptr), mPass(nullptr), mWvpVariable(nullptr), mTextureShaderResourceView(nullptr), mColorTextureVariable(nullptr),
    ...
    void Spikes::Initialize()
    {
       ...
       // std::wstring textureName = L"Content\\Textures\\EarthComposite.jpg";
		...
		std::wstring textureName = L"Content\\Textures\\black.jpg"; 
...
*/
	...
	void Spikes::SetPosition(const float rotateX, const float rotateY, const float rotateZ, const float scaleFactor, const float translateX, const float translateY, const float translateZ)
	{
		XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
	...
	void Spikes::Update(const GameTime& gameTime)
	{
	//	XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
    ...
    void Spikes::Draw(const GameTime& gameTime)
    {
    ...
    void Spikes::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
    {

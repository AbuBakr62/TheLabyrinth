...
namespace Rendering
{
    RTTI_DEFINITIONS(Clouds)

    Clouds::Clouds(Game& game, Camera& camera, const std::string modelFilename)
	...
	Clouds::Clouds(Game& game, Camera& camera, const std::string modelFilename, const std::wstring ModelDes, int ModelValue)
		: DrawableGameComponent(game, camera),
		mEffect(nullptr), mTechnique(nullptr), mPass(nullptr), mWvpVariable(nullptr), mTextureShaderResourceView(nullptr), mColorTextureVariable(nullptr),
    ...
    void Clouds::Initialize()
    {
       ...
       // std::wstring textureName = L"Content\\Textures\\EarthComposite.jpg";
		...
		std::wstring textureName = L"Content\\Textures\\Clouds.jpg"; 
...
*/
	...
	void Clouds::SetPosition(const float rotateX, const float rotateY, const float rotateZ, const float scaleFactor, const float translateX, const float translateY, const float translateZ)
	{
		XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
	...
	void Clouds::Update(const GameTime& gameTime)
	{
		XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
		mAngle += XM_PI * static_cast<float> (gameTime.ElapsedGameTime());
		
		mAngle = 0.000002 * XM_PI;
    ...
    void Clouds::Draw(const GameTime& gameTime)
    {
    ...
    void Clouds::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
    {

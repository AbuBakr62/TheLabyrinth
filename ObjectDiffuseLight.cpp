...
namespace Rendering
{
	...
	void ObjectDiffuseLight::SetPosition(const float rotateX, const float rotateY, const float rotateZ, const float scaleFactor, const float translateX, const float translateY, const float translateZ)
	{
	...
	void ObjectDiffuseLight::Initialize()
	{
		...
		std::unique_ptr<Model> model(new Model(*mGame, "Content\\Models\\map.obj", true));
		...
		std::wstring textureName = L"Content\\Textures\\grass2.jpg";
		...
		mDirectionalLight->ApplyRotation(XMMatrixRotationX(180));
		mAmbientColor.a = 160;
	...
	void ObjectDiffuseLight::UpdateDirectionalLight(const GameTime& gameTime)
	{
	...
	void ObjectDiffuseLight::UpdateAmbientLight(const GameTime& gameTime)
	{

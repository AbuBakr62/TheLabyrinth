...
namespace Rendering
{
	class Rocks : public DrawableGameComponent
	{
		RTTI_DECLARATIONS(Rocks, DrawableGameComponent)

	public:
		Rocks(Game& game, Camera& camera, const std::string modelFilename);
		Rocks(Game& game, Camera& camera, const std::string modelFilename, const std::wstring modelDes, int modelValue);
		~Rocks();
	...
	private:
		...
		Rocks();
		Rocks(const Rocks& rhs);
		Rocks& operator=(const Rocks& rhs);

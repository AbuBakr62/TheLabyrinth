...
namespace Rendering
{
	class Trees : public DrawableGameComponent
	{
		RTTI_DECLARATIONS(Trees, DrawableGameComponent)

	public:
		Trees(Game& game, Camera& camera, const std::string modelFilename);
		Trees(Game& game, Camera& camera, const std::string modelFilename, const std::wstring modelDes, int modelValue);
		~Trees();
